...
def main():
    ...
    val_iter = chainer.iterators.MultiprocessIterator(
        val, args.val_batchsize, repeat=False, n_processes=args.loaderjob, shuffle=True)
    ...
    trainer = training.Trainer(updater, (56310, 'iteration'), args.out)
    #trainer.extend(extensions.polynomial_shift.PolynomialShift('lr', (1, 320000)), trigger=(1, 'iteration'))
    warmup_start_lr = 0.1
    warmup_iter = 3125
    base_lr = 0.8
    gamma = 0.1
    trainer.extend(extensions.warmup_shift.WarmupShift('lr', warmup_start_lr, warmup_iter, base_lr), trigger=(1, 'iteration'))
    trainer.extend(extensions.ExponentialShift('lr', 0.1, init=0.8), trigger=triggers.ManualScheduleTrigger([18750, 37500, 50000], 'iteration'))
    #trainer.extend(extensions.ExponentialShift('lr', 0.08, init=1e-1), trigger=triggers.ManualScheduleTrigger([50, 60], 'iteration'))
    #trainer.extend(extensions.warmup_multistep.WarmUpMultistep('lr', base_lr, warmup_start_lr, warmup_iter, gamma, step_value), trigger=(1, 'iteration'))

    ####
    val_interval = (10 if args.test else 1877), 'iteration'
